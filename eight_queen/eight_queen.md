...
`neighbor`ポインタに左のクイーンを指させる. `find_sol`関数でオブジェクト同士の通信で左のやつに取られないような位置へ移動する. チェス盤の大きさは `max_row`と `queen_t`オブジェクトの数で決まる.
...
10x10のチェス盤にクイーンを置いた. Fortranには new 演算子みたいなものが(多分)なくて, 変数に新しいオブジェクトを代入してもアドレスが変わらないっぽい? なので, `queen_t` オブジェクトを配列で宣言した.
